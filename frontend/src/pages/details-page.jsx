...
export default function DetailsPage() {
  ...
  const images = post?.images && post.images.length > 0 ? post.images : [post?.imageLink];

  if (!loading)
    return (
      <div className="flex-grow bg-light dark:bg-dark">
        <div className="relative flex flex-col">
          <img src={images[0]} alt={post.title} className="h-80 w-full object-cover sm:h-96" />
        ...
        {images.length > 1 && (
          <div className="mx-auto max-w-4xl px-4 pb-10">
            <h2 className="mb-4 text-2xl font-semibold text-light-primary dark:text-dark-primary">Gallery</h2>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {images.slice(1).map((img, idx) => (
                <img
                  ...
                  src={img}
                  alt={`Gallery ${idx + 1}`}
                  className="h-64 w-full rounded-lg object-cover shadow-md transition-transform hover:scale-105"
                />
              ))}
            </div>
          </div>
